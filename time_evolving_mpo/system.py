...
def _liouvillian(hamiltonian, gammas, lindblad_operators):
    ...
    """
    liouvillian = -1j * commutator(hamiltonian) #this hamiltonian has not been vectorised, i.e. it's of shape (dim,dim)
...
class AdjointTimeDependentSystem(TimeDependentSystem):
    r"""
    Represents an explicitly time dependent system (without any coupling to a
    non-Markovian bath). It is possible to include (also explicitly
    time dependent) Lindblad terms in the master equation.
    ...
    def liouvillian_derivative(self, hamiltonian_derivative) -> ndarray:
        r"""
        Returns the Liouvillian super-operator :math:`\mathcal{L}(t)` with
        ...
        if self.gammas or self.lindblad_operators != []:
            raise NotImplementedError('Not supported --Eoin') #maybe i can just add this?
        return _liouvillian(hamiltonian_derivative, [], []) # this hamiltonian has not been vectorised, i.e. it's of shape (dim,dim)
